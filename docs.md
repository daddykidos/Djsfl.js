...
```javascript   
new vector2()      //returns {x: 0, y: 0}
new vector2(32)    //returns {x: 32, y:32}
new vector2(43,-3) //returns {x: 43, y: -3}
...
### getWindow() - returns window size in vector2 format
...
You need to call the elementHandler() function to refresh the actual visual position of all elements containing the  djsflElement className
    ...
    sizeState  //number 0-pixel | 1-width | 2-height | 3 - own axis | 4 - min axis of window

sizeState - is the choice of what scaling method you want with choices from sizeStates (js object containing different scaling methods)
...
As for **pos** and **posRatio** note that anchor is applied to each visually
...
as for size, sizeRatio will be the variable for processing if sizeState > 0 else it will assign the value of size in pixels
...
## elementInit(element) - initializes stats of a none djsflElement so that it would be treated as one

just put the obj/element then it will initialize the stats to default as if it was created newly with createElement()
